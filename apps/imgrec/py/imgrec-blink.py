    ...
    if clnt.get_run_co_sandbox():
        ...
        if use_ux:
            if use_async and not model_local_path:
                ...
                fut_img   = clnt.ux_delegated_get_file_view(1, async_=True)
                fut_model = clnt.ux_delegated_get_file_view(0, async_=True)
                ...
                img_bytes = clnt.ux_delegated_get_file_view(1)
                clnt.log_spawn_latency("Paper.Initialization.TransferImage",
                ...
                if model_local_path:
                    ...
                    model_bytes = bytes(clnt.ux_delegated_get_file_view(0))
                clnt.log_spawn_latency("Paper.Initialization.TransferModel",
        ...
        if use_ux:
            if use_async:
                fetch_start = time.perf_counter()
                fut_img = clnt.ux_get_file(img_key, async_=True)
                if not model_local_path:
                    fut_model = clnt.ux_get_file(model_key, async_=True)
                img_bytes = fut_img.result()
                ...
                img_start = time.perf_counter()
                img_bytes = clnt.ux_get_file(img_key)
                clnt.log_spawn_latency("Paper.Initialization.DownloadImage",
                                       int((time.perf_counter() - img_start) * 1_000_000))
                model_start = time.perf_counter()
                if model_local_path:
                    ...
                    model_bytes = clnt.ux_get_file(model_key)
                clnt.log_spawn_latency("Paper.Initialization.DownloadModel",
                                       int((time.perf_counter() - model_start) * 1_000_000))
        else:
            if use_async:
