...
func init() {
	flag.StringVar(&app, "app", "mr-wc.json", "application")
...
func runN(t *testing.T, em *crash.TeventMap, srvs map[string]crash.Tselector, maliciousMapper int, monitor bool) (int64, uint64, *spstats.TcounterSnapshot) {
	...
	cm := mrcoord.StartMRJob(sc, ts.jobRoot, ts.job, job, nmap, MEM_REQ, MEM_REQ, maliciousMapper, ts.tasks.Mftsrv.Id, ts.tasks.Rftsrv.Id)
