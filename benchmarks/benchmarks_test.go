...
func TestAppMR(t *testing.T) {
	...
	p := newRealmPerf(mrts.GetRealm(REALM1))
	defer p.Done()
	jobs, apps := newNMRJobs(mrts.GetRealm(REALM1), p, 1, MRBenchConfig.App, MRBenchConfig.JobCfg, chooseMRJobRoot(mrts.GetRealm(REALM1)), MRBenchConfig.MapperMem, MRBenchConfig.ReducerMem, MRBenchConfig.NDedicatedUxNodes)
	go func() {
		for _, j := range jobs {
			// Wait until ready
			<-j.ready
...
func TestRealmBalanceMRMR(t *testing.T) {
	...
	for i := range realms {
		...
		defer ps[i].Done()
		mrjob, mrapp := newNMRJobs(mrts.GetRealm(realms[i]), ps[i], 1, MRBenchConfig.App, MRBenchConfig.JobCfg, chooseMRJobRoot(mrts.GetRealm(realms[i])), MRBenchConfig.MapperMem, MRBenchConfig.ReducerMem, MRBenchConfig.NDedicatedUxNodes)
		mrjobs[i] = mrjob
		mrapps[i] = mrapp
	}
