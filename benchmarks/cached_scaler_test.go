...
func NewCachedScalerJob(ts *test.RealmTstate, jobName string, cacheCfg *benchmarks.CacheBenchConfig, cosSimCfg *benchmarks.CosSimBenchConfig) *CachedScalerJobInstance {
	...
	ji.doPuts = !(len(ji.cacheCfg.PutMaxRPS) == 1 && ji.cacheCfg.PutMaxRPS[0] == 0)
	...
	ji.primaryEPs = make([]*sp.Tendpoint, ji.cacheCfg.JobCfg.NSrv)
	for i := 0; i < ji.cacheCfg.JobCfg.NSrv; i++ {
	...
	if !foundSleeper && ji.cacheCfg.RunSleeper {
		db.DPrintf(db.TEST, "Didn't find sleeper")
	}
	if !assert.True(ts.Ts.T, foundCached, "Err didn't find cached srv") {
		return ji
	}
	if !assert.True(ts.Ts.T, !ji.cacheCfg.RunSleeper || foundSleeper, "Err didn't find sleeper srv") {
		return ji
	}

	// Construct CosSim request input
	cossimInputVec := cossim.VectorToSlice(cossim.NewVectors(1, ji.cosSimCfg.JobCfg.VecDim)[0])
	ranges := []*cossimproto.VecRange{
		&cossimproto.VecRange{
			StartID: 0,
			EndID:   uint64(ji.cosSimCfg.NVecToQuery - 1),
		},
	}
	...
	ji.lgs = make([]*loadgen.LoadGenerator, 0, len(ji.cacheCfg.Durs))
	for i := range ji.cacheCfg.Durs {
		ji.lgs = append(ji.lgs, loadgen.NewLoadGenerator(ji.cacheCfg.Durs[i], ji.cacheCfg.MaxRPS[i], func(r *rand.Rand) (time.Duration, bool) {
	...
	if ji.doPuts {
		ji.putLGs = make([]*loadgen.LoadGenerator, 0, len(ji.cacheCfg.PutDurs))
		for i := range ji.cacheCfg.PutDurs {
			ji.putLGs = append(ji.putLGs, loadgen.NewLoadGenerator(ji.cacheCfg.PutDurs[i], ji.cacheCfg.PutMaxRPS[i], func(r *rand.Rand) (time.Duration, bool) {
				idx := r.Int() % len(ji.keys)
				// Select a key to request
				key := ji.keys[idx]
				val := ji.vals[idx]
	...
	if ji.cacheCfg.CosSimBackend {
		...
		ji.cossimJ, err = cossimsrv.NewCosSimJob(ji.cosSimCfg.GetJobConfig(), ts.SigmaClnt, ji.epcj, ji.cm, ji.cc)
...
func (ji *CachedScalerJobInstance) StartCachedScalerJob() {
	db.DPrintf(db.ALWAYS, "Start cached scaler job cfg:%v", ji.cacheCfg)
	ji.warmup = true
	...
	go func() {
		...
		for i, putLG := range ji.putLGs {
			db.DPrintf(db.TEST, "Run put load generator rps %v dur %v", ji.cacheCfg.PutMaxRPS[i], ji.cacheCfg.PutDurs[i])
			putLG.Run()
		}
	}()
	for i, lg := range ji.lgs {
		db.DPrintf(db.TEST, "Run get load generator rps %v dur %v", ji.cacheCfg.MaxRPS[i], ji.cacheCfg.Durs[i])
		lg.Run()
	}
	wg.Wait()
}
