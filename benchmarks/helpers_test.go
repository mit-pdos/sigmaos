...
func makeHotelJobs(ts *test.Tstate, sigmaos bool, ncore proc.Tcore, dur time.Duration, maxrps int, fn hotelFn) ([]*HotelJobInstance, []interface{}) {
	// n is ntrials, which is always 1.
	n := 1
	ws := make([]*HotelJobInstance, 0, n)
	is := make([]interface{}, 0, n)
	for i := 0; i < n; i++ {
		i := MakeHotelJob(ts, sigmaos, ncore, dur, maxrps, fn)
		ws = append(ws, i)
		is = append(is, i)
	}
	return ws, is
