	...
	if ji.cfg.CacheBenchCfg.Migrate.Migrate {
		go func() {
			...
			for i := 0; i < len(delays) && i < len(targets); i++ {
				...
				if err := ji.hj.CacheMgr.MigrateServerWithSigmaPath(ji.hj.CacheClnt.GetCacheClnt(), chunk.ChunkdPath(ji.warmCachedSrvKID), ji.cfg.CacheBenchCfg.UseCoSandbox, targets[i]); !assert.Nil(ji.Ts.T, err, "Err migrate cache server: %v", err) {
					return
				}
