...
func (ts *Tstate) RunParallelClientBenchmark(benchName string, driverVMs []int, getLeaderClientBenchCmd GetBenchCmdFn, getFollowerClientBenchCmd GetBenchCmdFn, startK8sApp K8sAppMgmtFn, stopK8sApp K8sAppMgmtFn, clientDelay time.Duration, numNodes int, numCoresPerNode uint, numFullNodes int, numProcqOnlyNodes int, turboBoost bool, useGVisor bool) bool {
	...
	// Warn periodically if the benchmark takes unreasonably long, so that a run
	// which has wedged is distinguishable from one which is merely slow.
	wd := startWatchdog(benchName)
	defer wd.Stop()
	start = time.Now()
	// First, stop any previously running cluster
	wd.SetPhase("stop previous cluster")
	...
	if !ts.BCfg.NoShutdown {
		defer func() {
			// Stop the SigmaOS cluster once the benchmark is over. Runs before
			// the watchdog's deferred Stop (defers are LIFO), so a shutdown which
			// hangs is still reported.
			wd.SetPhase("stop cluster")
	...
	start = time.Now()
	wd.SetPhase("run benchmark clients")
...
//
// If any of the above steps results in an error, bail out early.
func (ts *Tstate) RunStandardBenchmark(benchName string, driverVM int, getBenchCmd GetBenchCmdFn, numNodes int, numCoresPerNode uint, numFullNodes int, numProcqOnlyNodes int, turboBoost bool, useGVisor bool) {
	ts.RunParallelClientBenchmark(benchName, []int{driverVM}, getBenchCmd, nil, nil, nil, 0*time.Second, numNodes, numCoresPerNode, numFullNodes, numProcqOnlyNodes, turboBoost, useGVisor)
}
