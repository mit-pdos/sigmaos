...
run_mr() {
  ...
  cmd="
    ...
    $set_perf; \
    go clean -testcache; \
    go test -v sigmaos/benchmarks -timeout 0 $OVERLAYS --tag $TAG --etcdIP $LEADER_IP_SIGMA --run AppMR $prewarm --mrapp $mrapp --mr_mem_req $mem_req --mr_asyncrw $NETPROXY > /tmp/bench.out 2>&1
  "
...
run_socialnet() {
  ...
  cmd="
    ...
    go clean -testcache; \
    ulimit -n 100000; \
    go test -v sigmaos/benchmarks -timeout 0 $OVERLAYS --tag $TAG --etcdIP $LEADER_IP_SIGMA --run $testname --k8saddr $k8saddr --prewarm_realm --sn_dur $dur --sn_max_rps $rps --sn_read_only $NETPROXY > /tmp/bench.out 2>&1
  "
  if [ "$sys" = "Sigmaos" ]; then
    vpc=$VPC
  else
...
run_cached() {
  ...
  cmd="
    go clean -testcache; \
    go test -v sigmaos/benchmarks -timeout 0 $OVERLAYS -run AppCached --nkvd $nkvd --kvd_mcpu $kvd_mcpu --nclerk $nclerk $NETPROXY > /tmp/bench.out 2>&1
  "
  run_benchmark $VPC $n_cores "" $n_vm $perf_dir "$cmd" 0 true false "swapoff"
}
...
#  "
  cmd="
    export SIGMADEBUG=\"TEST;BENCH;MR;\"; \
    go clean -testcache; \
    go test -v sigmaos/benchmarks -timeout 0 $OVERLAYS --tag $TAG --etcdIP $LEADER_IP_SIGMA --run RealmBalanceMRMR --sleep $sl --mrapp $mrapp --nrealm $n_realm --mr_mem_req $mem_req $NETPROXY > /tmp/bench.out 2>&1
#  "
...
#    ./start-k8s-app.sh --vpc $KVPC --path "corral/k8s20G-$i" --nrunning $np
  ...
  cmd="
    ...
    go test -v sigmaos/benchmarks -timeout 0 $OVERLAYS --tag $TAG --etcdIP $LEADER_IP_SIGMA --run K8sMRMulti --k8sleaderip $k8sleaderip --s3resdir $s3dir --sleep $sl --nrealm $n_realm $NETPROXY > /tmp/bench.out 2>&1
  "
...
#  hotel_dur="20s,20s,20s"
  ...
  cmd="
    ...
    go clean -testcache; \
    go test -v sigmaos/benchmarks -timeout 0 $OVERLAYS --tag $TAG --etcdIP $LEADER_IP_SIGMA --run RealmBalanceMRHotel --sleep $sl --hotel_dur $hotel_dur --hotel_max_rps $hotel_max_rps --hotel_ncache $hotel_ncache --mrapp $mrapp $NETPROXY > /tmp/bench.out 2>&1
  "
  run_benchmark $VPC 4 "" $n_vm $perf_dir "$cmd" $driver_vm true false "swapoff"
}
...
###
  ...
  cmd="
    ...
    go clean -testcache; \
    go test -v sigmaos/benchmarks $OVERLAYS -timeout 0 --tag $TAG --etcdIP $LEADER_IP_SIGMA --run RealmBalanceMRHotel --sleep $sl --hotel_dur $hotel_dur --hotel_max_rps $hotel_max_rps --hotel_ncache $hotel_ncache --mrapp $mrapp $bmem --nclnt $n_clnt_vms $NETPROXY > /tmp/bench.out 2>&1
  "
...
k8s_balance() {
  ...
  cmd="
    ...
    go clean -testcache; \
    echo get ready to run ; \
    go test -v sigmaos/benchmarks -timeout 0 $OVERLAYS --tag $TAG --etcdIP $LEADER_IP_SIGMA --run K8sBalanceHotelMR --hotel_dur $hotel_dur --hotel_max_rps $hotel_max_rps --k8sleaderip $k8sleaderip --k8saddr $k8saddr --s3resdir $s3dir $NETPROXY > /tmp/bench.out 2>&1
  "
  run_benchmark $KVPC 4 "" $n_vm $perf_dir "$cmd" $driver_vm true false "swapoff"
  cd $SCRIPT_DIR
  echo "Stopping hotel"
...
k8s_balance_multi() {
  ...
  cmd="
    ...
    go clean -testcache; \
    echo get ready to run ; \
    go test -v sigmaos/benchmarks $OVERLAYS -timeout 0 --tag $TAG --etcdIP $LEADER_IP_SIGMA --run K8sBalanceHotelMR --hotel_dur $hotel_dur --hotel_max_rps $hotel_max_rps --k8sleaderip $k8sleaderip --k8saddr $k8saddr --s3resdir $s3dir --nclnt $n_clnt_vms $NETPROXY > /tmp/bench.out 2>&1
  "
...
mr_k8s() {
  ...
  cmd="
    ...
    go clean -testcache; \
    go test -v sigmaos/benchmarks $OVERLAYS -timeout 0 --tag $TAG --etcdIP $LEADER_IP_SIGMA --run MRK8s --k8sleaderip $k8saddr --s3resdir $s3dir $NETPROXY > /tmp/bench.out 2>&1
  "
  run_benchmark $KVPC 4 "" $n_vm $perf_dir "$cmd" $driver_vm true false "swapoff"
}
...
#  imgpath="name/ux/~local/9ps3/img/6.jpg"
  ...
  cmd="
    ...
    go clean -testcache; \
    go test -v sigmaos/benchmarks $OVERLAYS -timeout 0 --tag $TAG --etcdIP $LEADER_IP_SIGMA --run TestImgResize --n_imgresize $n_imgresize --imgresize_nround $imgresize_nrounds --imgresize_path $imgpath --imgresize_mcpu $mcpu --imgresize_mem $imgresize_mem $NETPROXY > /tmp/bench.out 2>&1
  "
  run_benchmark $VPC 4 "" $n_vm $perf_dir "$cmd" $driver_vm true false "swapoff"
}
...
k8s_img_resize() {
  ...
  for i in 10 20 40 80 160 320 ; do
    ...
    cmd="
      ...
      go test -v sigmaos/benchmarks $OVERLAYS -timeout 0 --tag $TAG --etcdIP $LEADER_IP_SIGMA --run TestK8sImgResize $NETPROXY > /tmp/bench.out 2>&1
    "
    run_benchmark $KVPC 4 "" $n_vm $perf_dir "$cmd" $driver_vm true false "swapoff"
  done
}
...
#  prewarm=""
  ...
  for rps in 200 400 600 800 1000 1200 1400 1600 ; do
    ...
    cmd="
      ...
      go clean -testcache; \
      go test -v sigmaos/benchmarks $OVERLAYS -timeout 0 --run TestMicroScheddSpawn --tag $TAG --schedd_dur $dur --schedd_max_rps $rps --etcdIP $LEADER_IP_SIGMA --no-shutdown $prewarm $NETPROXY > /tmp/bench.out 2>&1
    "
    # Start driver VM asynchronously.
...
#  for qps_per_machine in 200 400 600 800 1000 1200 1400 1600 1800 2000 2200 2400; do
  for qps_per_machine in 1 ; do
    ...
    cmd="
      ...
      ./set-cores.sh --set 1 --start 2 --end 39 > /dev/null 2>&1 ; \
      go clean -testcache; \
      go test -v sigmaos/benchmarks -timeout 0 $OVERLAYS --run TestMicroScheddSpawn --tag $TAG --schedd_dur $dur --schedd_max_rps $rps --use_rust_proc --etcdIP $LEADER_IP_SIGMA $prewarm --no-shutdown $NETPROXY --with_kernel_pref > /tmp/bench.out 2>&1
    "
    # Start driver VM asynchronously.
    run_benchmark $VPC $ncore $turbo $n_vm $perf_dir "$cmd" $driver_vm true true false
    # Wait for test to terminate.
