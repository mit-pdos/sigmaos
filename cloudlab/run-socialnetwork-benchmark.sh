...
usage() {
  echo "Usage: $0 [--version VERSION] [--pull IMAGE_LABEL]" 1>&2
}
...
#
	...
	COMMAND="
		go clean -testcache;
		kubectl delete -Rf /tmp/thumbnail.yaml
		frontend_ip=
...
graph_simaos_simple() {
	...
	python3 $GRAPH_SCRIPTS_DIR/aggregate-tpt-simple.py --measurement_dir $PERF_DIR --mr_realm benchrealm1 --hotel_realm benchrealm2 --total_ncore 4 --title "Core Utilization of 2 Realms' Applications" --out $GRAPH_OUT_DIR/sigma-simple-$VERSION-result
}
...
graph_sigmaos_sn() {
	...
	cd $GRAPH_SCRIPTS_DIR
	echo "========== Graphing $PERF_DIR =========="
	python3 $GRAPH_SCRIPTS_DIR/aggregate-tpt.py --measurement_dir $PERF_DIR --mr_realm benchrealm1 --hotel_realm benchrealm2 --total_ncore 14 --units "Latency (ms),Req/sec,MB/sec" --title "Aggregate Throughput Balancing 2 Realms' Applications" --out $GRAPH_OUT_DIR/sigma-$VERSION-result
}
...
graph_k8s_sn() {
	...
	python3 $GRAPH_SCRIPTS_DIR/aggregate-tpt.py --measurement_dir $PERF_DIR_K8S --mr_realm benchrealm1 --hotel_realm benchrealm2 --total_ncore 14 --units "Latency (ms),Req/sec,MB/sec" --title "Aggregate Throughput Balancing 2 Realms' Applications" --out $GRAPH_OUT_DIR/k8s-$VERSION-result
}
