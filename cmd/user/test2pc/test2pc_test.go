...
func (ts *Tstate) spawnParticipant(index, opcode string) string {
	p := proc.MakeProc("test2pc", []string{index, opcode})
	ts.Spawn(p)
	return p.Pid
}

func (ts *Tstate) spawnCoord(opcode string, fws []string) string {
	p := proc.MakeProc("twopc-coord", append([]string{opcode}, fws...))
	ts.Spawn(p)
...
func TestCrash3(t *testing.T) {
	...
	gmcoord := groupmgr.Start(ts.System.FsLib, ts.System.ProcClnt, NCOORD, "twopc-coord", args, NCOORD, 0)
