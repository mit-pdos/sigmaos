...
class Srv {
  ...
  void fetch_init_vectors_from_cache(std::shared_ptr<std::promise<std::expected<int, sigmaos::serr::Error>>> result, int srv_id, std::vector<std::string> &key_vec, std::vector<int> &key_vec_int);
};
