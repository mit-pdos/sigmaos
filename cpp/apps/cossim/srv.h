...
class Srv {
 public:
  Srv(std::shared_ptr<sigmaos::proxy::sigmap::Clnt> sp_clnt, int nvec,
      int vec_dim, std::string cache_clnt_pn, int ncache, bool eager_init)
      : _mu(),
        _nvec(nvec),
        _vec_dim(vec_dim),
        _vec_db(),
    ...
    auto cossim_ep = std::make_shared<sigmaos::rpc::srv::RPCEndpoint>(
        "CosSimSrv.CosSim", std::make_shared<CosSimReq>(),
        std::make_shared<CosSimRep>(),
        std::bind(&Srv::CosSim, this, std::placeholders::_1,
                  std::placeholders::_2));
 ...
 private:
  ...
  // CosSim RPC handler
  std::expected<int, sigmaos::serr::Error> CosSim(
