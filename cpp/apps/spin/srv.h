...
class Srv {
 public:
  Srv(std::shared_ptr<sigmaos::proxy::sigmap::Clnt> sp_clnt, bool use_epcache)
      : _sp_clnt(sp_clnt), _use_epcache(use_epcache) {
    ...
    auto spin_ep = std::make_shared<sigmaos::rpc::srv::RPCEndpoint>(
        "SpinSrv.Spin", std::make_shared<SpinReq>(),
        std::make_shared<SpinRep>(),
        std::bind(&Srv::Spin, this, std::placeholders::_1,
                  std::placeholders::_2));
    _srv->ExposeRPCHandler(spin_ep);
    log(SPINSRV, "Exposed spin RPC handler");
    {
      auto pn = SPINSRV_UNION_DIR_PN /
                std::filesystem::path(_sp_clnt->ProcEnv()->GetPID());
      auto start = GetCurrentTime();
      ...
      LogSpawnLatency(_sp_clnt->ProcEnv()->GetPID(),
                      ...
                      std::format("RegisterEP epcache {}", _use_epcache));
 ...
 private:
  ...
  // Spin RPC handler
  std::expected<int, sigmaos::serr::Error> Spin(
