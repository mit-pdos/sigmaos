...
const std::string SPPROXY_SOCKET_PN =
    "/tmp/spproxyd/spproxyd.sock";  // sigmap/sigmap.go SIGMASOCKET
...
class Clnt {
 public:
  ...
  std::expected<std::shared_ptr<TstatProto>, sigmaos::serr::Error> Stat(
      std::string pn);
  std::expected<int, sigmaos::serr::Error> Create(
      std::string pn, sigmaos::sigmap::types::Tperm perm,
      sigmaos::sigmap::types::Tmode mode);
  std::expected<int, sigmaos::serr::Error> Open(
      std::string pn, sigmaos::sigmap::types::Tmode mode, bool wait);
  std::expected<int, sigmaos::serr::Error> Rename(std::string src,
                                                  std::string dst);
  ...
  std::expected<std::shared_ptr<std::string>, sigmaos::serr::Error> GetFile(
      std::string pn);
  std::expected<uint32_t, sigmaos::serr::Error> PutFile(
      std::string pn, sigmaos::sigmap::types::Tperm perm,
  ...
  std::expected<uint32_t, sigmaos::serr::Error> Pread(
  ...
  std::expected<int, sigmaos::serr::Error> Seek(
      int fd, sigmaos::sigmap::types::Toffset offset);
  // TODO: fence type in CreateLeased?
  std::expected<int, sigmaos::serr::Error> CreateLeased(
      std::string path, sigmaos::sigmap::types::Tperm perm,
      sigmaos::sigmap::types::Tmode mode,
  ...
  std::expected<bool, sigmaos::serr::Error> IsLocalMount(
      std::shared_ptr<TendpointProto> ep);
  std::expected<std::pair<std::vector<std::string>, std::vector<std::string>>,
                sigmaos::serr::Error>
  PathLastMount(std::string pn);
  std::expected<std::shared_ptr<TendpointProto>, sigmaos::serr::Error>
  GetNamedEndpoint();
  std::expected<int, sigmaos::serr::Error>
  InvalidateNamedEndpointCacheEntryRealm(sigmaos::sigmap::types::Trealm realm);
  std::expected<std::shared_ptr<TendpointProto>, sigmaos::serr::Error>
  GetNamedEndpointRealm(sigmaos::sigmap::types::Trealm realm);
  std::expected<int, sigmaos::serr::Error> NewRootMount(std::string pn,
                                                        std::string mntname);
  std::expected<std::vector<std::string>, sigmaos::serr::Error> Mounts();
  std::expected<int, sigmaos::serr::Error> SetLocalMount(
  ...
  std::expected<int, sigmaos::serr::Error> RegisterEP(
      std::string pn, std::shared_ptr<TendpointProto> ep);

  // ========== ProcClnt API ==========
  std::expected<int, sigmaos::serr::Error> Started();
  std::expected<int, sigmaos::serr::Error> Exited(sigmaos::proc::Tstatus status,
                                                  std::string &msg);
 ...
 private:
  ...
  bool _disconnected;
  bool _use_shmem_writeread = false;
