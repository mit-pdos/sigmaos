...
std::expected<int, sigmaos::serr::Error> Channel::Init() {
  ...
  std::string clone_pn = _srv_pn + "/" + sigmaos::rpc::CLONE;
  {
    ...
    if (!res.has_value()) {
      log(SPCHAN_ERR, "Error GetFile clone file({}): {}", clone_pn,
          res.error().String());
  ...
  {
    std::string sess_dev_pn =
        _srv_pn + "/" + sigmaos::rpc::RPC + "/" + _sid + "/data";
    auto res =
        _sp_clnt->Open(sess_dev_pn, sigmaos::sigmap::constants::ORDWR, false);
    if (!res.has_value()) {
      log(SPCHAN_ERR, "Error Open data file({}): {}", sess_dev_pn,
          res.error().String());
      return std::unexpected(res.error());
    }
    _fd = res.value();
  ...
  _initialized = true;
  log(SPCHAN, "Successfully initialized sigmap-based RPC channel to {}",
      _srv_pn);
  return 0;
}
...
std::expected<std::shared_ptr<sigmaos::io::transport::Call>,
              sigmaos::serr::Error>
Channel::SendReceive(std::shared_ptr<sigmaos::io::transport::Call> call) {
  // Sanity check
  if (!_initialized) {
...
bool Channel::IsClosed() { return _closed; }

bool Channel::IsInitialized() { return _initialized; }
