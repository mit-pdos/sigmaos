...
func MakeFdClient(fsc *fidclnt.FidClnt, uname string, realm sp.Trealm, lip string, sz sessp.Tsize) *FdClient {
	fdc := &FdClient{}
	fdc.PathClnt = pathclnt.MakePathClnt(fsc, realm, lip, sz)
	fdc.fds = mkFdTable()
	fdc.uname = uname
	return fdc
}
