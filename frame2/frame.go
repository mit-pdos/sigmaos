...
func WriteFrame(wr io.Writer, frame sessp.Tframe) *serr.Err {
	...
	if len(frame) == 0 || nbyte == 0 {
		db.DPrintf(db.ERROR, "frame write [%p] BAD len: %v %v", wr, len(frame), uint32(len(frame)+4))
	}
...
func WriteNumOfFrames(wr io.Writer, nframes uint32) *serr.Err {
	// if nframes == 0 {
	// 	db.DPrintf(db.ERROR, "frame write [%p] number of frames: %v", wr, nframes)
	// }
