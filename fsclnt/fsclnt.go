...
func (fsc *FsClient) DPrintf(format string, a ...interface{}) {
	if fsc.debug {
		log.Printf(format, a...)
	}
}
...
func (fsc *FsClient) walkOne(path []string) (np.Tfid, int, error) {
	...
	fsc.DPrintf("walkOne rest %v -> %v %v", rest, reply.Qids, todo)
