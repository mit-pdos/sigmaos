...
func TestServerPerf(t *testing.T) {
	...
	mgr, err := fttasksrv.NewFtTaskSrvMgr(ts.SigmaClnt, "test", nil, true)
	assert.Nil(t, err)
...
func runTestServerData(t *testing.T, em *crash.TeventMap) []*procgroupmgr.ProcStatus {
	...
	mgr, err := fttasksrv.NewFtTaskSrvMgr(ts.SigmaClnt, "test", em, true)
	assert.Nil(t, err)
...
func TestServerPartition(t *testing.T) {
	...
	mgr, err := fttasksrv.NewFtTaskSrvMgr(ts.SigmaClnt, "test", nil, false)
	assert.Nil(t, err)
