...
func NewNewTaskMgr[Data any, Output any](pclnt procapi.ProcAPI, ft *ftclnt.FtTaskClnt[Data, Output]) (*NewFtTaskMgr[Data, Output], error) {
	...
	return &NewFtTaskMgr[Data, Output]{ProcAPI: pclnt, FtTaskClnt: ft}, nil
}

func (ftm *NewFtTaskMgr[Data, Output]) ExecuteTasks(new NewTnew[Data], mkProc NewTmkProc[Data]) *proc.Status {
	var r *NewTresult
...
func (ftm *NewFtTaskMgr[Data, Output]) getTasks(chTask chan<- []ftclnt.TaskId, chStop chan<- bool) {
	for {
...
func (ftm *NewFtTaskMgr[Data, Output]) startTasks(tasks []ftclnt.TaskId, ch chan NewTresult, new NewTnew[Data], mkProc NewTmkProc[Data]) error {
	ntask := 0
...
func (ftm *NewFtTaskMgr[Data, Output]) runTask(p *proc.Proc, t ftclnt.TaskId, ch chan<- NewTresult) {
...
func (ftm *NewFtTaskMgr[Data, Output]) waitForTask(start time.Time, p *proc.Proc, id ftclnt.TaskId) NewTresult {
