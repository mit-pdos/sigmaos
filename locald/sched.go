...
func (ld *LocalD) ClaimRunQJob(queuePath string, pid string) ([]byte, bool) {
	...
	err := ld.Rename(path.Join(queuePath, pid), path.Join(proc.CLAIMED, pid))
	if err != nil {
		return []byte{}, false
	}
	// Create an ephemeral file to mark that locald hasn't crashed
	err = ld.MakeFile(path.Join(proc.CLAIMED_EPH, pid), 0777|np.DMTMP, np.OWRITE, []byte{})
	if err != nil {
		log.Printf("Error making ephemeral claimed job file: %v", err)
	}
	b, _, err := ld.GetFile(path.Join(proc.CLAIMED, pid))
	if err != nil {
