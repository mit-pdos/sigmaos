...
func MakeReplMemFs(addr string, path string, name string, conf repl.Config) (*sesssrv.SessSrv, *sessp.Err) {
...
func MakeReplMemFsFsl(addr string, path string, fsl *fslib.FsLib, pclnt *procclnt.ProcClnt, conf repl.Config) (*sesssrv.SessSrv, *sessp.Err) {
	root := dir.MkRootDir(ctx.MkCtx("", 0, nil), memfs.MakeInode)
