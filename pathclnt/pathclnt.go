...
func (pathc *PathClnt) Create(p string, perm np.Tperm, mode np.Tmode) (np.Tfid, error) {
	...
	dir := path.Dir()
	base := path.Base()
	fid, err := pathc.Walk(dir, true, nil)
	if err != nil {
		return np.NoFid, err
	}
...
func (pathc *PathClnt) Rename(old string, new string) error {
	...
	fid, err := pathc.Walk(opath, np.EndSlash(old), nil)
	if err != nil {
		return err
	}
...
func (pathc *PathClnt) renameat(old, new string) *np.Err {
	...
	o := opath[len(opath)-1]
	n := npath[len(npath)-1]
	fid, err := pathc.Walk(opath[:len(opath)-1], np.EndSlash(old), nil)
	if err != nil {
		return err
	}
	defer pathc.FidClnt.Clunk(fid)
	fid1, err := pathc.Walk(npath[:len(npath)-1], np.EndSlash(old), nil)
	if err != nil {
		return err
	}
...
func (pathc *PathClnt) Remove(name string) error {
	...
	if err != nil {
		if np.IsMaybeSpecialElem(err) || np.IsErrUnreachable(err) {
			fid, err = pathc.Walk(path, np.EndSlash(name), nil)
			if err != nil {
				return err
			}
			defer pathc.FidClnt.Clunk(fid)
...
func (pathc *PathClnt) Stat(name string) (*np.Stat, error) {
	...
	if len(rest) == 0 && !np.EndSlash(name) {
		...
		fid, err := pathc.Walk(np.Split(name), np.EndSlash(name), nil)
		if err != nil {
			return nil, err
		}
		defer pathc.FidClnt.Clunk(fid)
...
func (pathc *PathClnt) OpenWatch(path string, mode np.Tmode, w Watch) (np.Tfid, error) {
	...
	p := np.Split(path)
	fid, err := pathc.Walk(p, np.EndSlash(path), w)
	if err != nil {
		return np.NoFid, err
	}
...
func (pathc *PathClnt) SetRemoveWatch(path string, w Watch) error {
	...
	p := np.Split(path)
	fid, err := pathc.Walk(p, np.EndSlash(path), nil)
	if err != nil {
		return err
	}
	if w == nil {
