...
func (pd *Procd) hasEnoughCores(p *proc.Proc) bool {
	...
	if p.Type == proc.T_LC {
		...
		if util < np.Conf.Procd.BE_PROC_CLAIM_CPU_THRESHOLD && /* load[0] < float64(linuxsched.NCores) && */ !(cload[0]-cload[1] >= 10.0 && cload[1]-cload[2] >= 10.0) && rlc {
			db.DPrintf(db.ALWAYS, "Claimed BE proc: util %v Linux load %v Custom load %v c1 %v c2 %v rate-limit check %v proc %v", util, load, cload, cload[0]-cload[1], cload[1]-cload[2], rlc, p.Program)
			return true
		} else {
