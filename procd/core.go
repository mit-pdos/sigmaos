...
func (pd *Procd) hasEnoughCores(p *proc.Proc) bool {
	...
	if p.Type == proc.T_LC {
		...
		if util < np.Conf.Procd.BE_PROC_CLAIM_CPU_THRESHOLD && !(cload[0]-cload[1] >= 10.0 && cload[1]-cload[2] >= 10.0) && !(cload[0] >= 95.0 && cload[1] >= 95.0 && cload[2] >= 95.0) && !(util-cload[0] >= 20.0) && rlc {
