...
func (pclnt *ProtClnt) SetFile(fid np.Tfid, path []string, mode np.Tmode, offset np.Toffset, data []byte) (*np.Rwrite, *np.Err) {
	args := np.Tsetfile{fid, mode, offset, path, data}
	reply, err := pclnt.Call(args)
	if err != nil {
		return nil, err
	}
