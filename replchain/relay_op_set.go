...
type RelayOpSet struct {
	mu      sync.Mutex
	entries map[sessp.Tsession]map[sessp.Tseqno][]*RelayOp
}

func MakeRelayOpSet() *RelayOpSet {
	s := &RelayOpSet{}
	s.entries = map[sessp.Tsession]map[sessp.Tseqno][]*RelayOp{}
	return s
}
...
func (s *RelayOpSet) Add(op *RelayOp) {
	...
	if _, ok := s.entries[op.request.Session]; !ok {
		s.entries[op.request.Session] = map[sessp.Tseqno][]*RelayOp{}
	}
