...
func (g *ProcdMgr) CheckpointProc(pid sp.Tpid, r sp.Trealm, pn string) error {
	db.DPrintf(db.PROCDMGR, "CheckpointProc %v", pid)
	rpcc, err := g.lookupClnt(r, proc.T_BE)
	if err != nil {
		return err
	}
	_, err = rpcc.CheckpointProc(pid, pn)
	return err
}
