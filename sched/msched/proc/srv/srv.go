...
import (
	...
	"sigmaos/fs"
	"sigmaos/kernelclnt"
	lazypagesclnt "sigmaos/lazypages/clnt"
...
func RunProcSrv(kernelId string, dialproxy bool, spproxydPID sp.Tpid) error {
	...
	lsdp.InheritParentProcEnv(ps.pe)
	lsdp.SetHow(proc.HLINUX)
	lpsd, err := lazypagessrv.ExecLazyPagesSrv(lsdp, ps.pe.GetInnerContainerIP(), ps.pe.GetOuterContainerIP(), sp.NOT_SET)
	if err != nil {
		return nil
	}

	lpc, err := lazypagesclnt.NewLazyPagesClnt(ps.sc.FsLib, lsdpid)
	if err != nil {
		db.DPrintf(db.PROCD, "NewLazyPagesClnt %v err %v", lsdpid, err)
		return nil
	}
	ps.lpc = lpc
