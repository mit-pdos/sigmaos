...
func TestServerCrash(t *testing.T) {
	...
	ch := make(chan error)
	go func() {
		fsl, err := fslib.MakeFsLibAddr("fslibtest-1", ts.NamedAddr())
		assert.Nil(t, err)
...
func TestServerPartitionNonBlocking(t *testing.T) {
	...
	for i := 0; i < N; i++ {
		ch := make(chan error)
		go func(i int) {
			fsl, err := fslib.MakeFsLibAddr(fmt.Sprintf("test-fsl-%v", i), ts.NamedAddr())
			assert.Nil(t, err)
			for true {
				_, err := fsl.Stat(DIRGRP0)
				if err != nil {
...
func TestServerPartitionBlocking(t *testing.T) {
	...
	for i := 0; i < N; i++ {
		ch := make(chan error)
		go func() {
			fsl, err := fslib.MakeFsLibAddr("fsl", ts.NamedAddr())
			assert.Nil(t, err)
			sem := semclnt.MakeSemClnt(fsl, DIRGRP0+"sem")
			sem.Init(0)
			err = sem.Down()
...
func TestWriteCrash(t *testing.T) {
	...
	for i := 0; i < N; i++ {
		go writer(ts.T, ch, strconv.Itoa(i), ts.NamedAddr())
	}
