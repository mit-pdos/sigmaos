...
type SessCond struct {
	...
	conds       map[fcall.Tsession][]*cond
	wakingConds map[fcall.Tsession]map[*cond]bool // Conds pending wakeup (which also may need to be alerted that the session has closed)
}
...
func makeSessCond(sct *SessCondTable, lock sync.Locker) *SessCond {
	...
	sc.sct = sct
	sc.lock = lock
	sc.conds = make(map[fcall.Tsession][]*cond)
	sc.wakingConds = make(map[fcall.Tsession]map[*cond]bool)
	return sc
}

func (sc *SessCond) alloc(sessid fcall.Tsession) *cond {
...
func (sc *SessCond) Wait(sessid fcall.Tsession) *fcall.Err {
	c := sc.alloc(sessid)
...
func (sc *SessCond) addWakingCond(sid fcall.Tsession, c *cond) {
...
func (sc *SessCond) removeWakingCond(sid fcall.Tsession, c *cond) {
...
func (sc *SessCond) closed(sessid fcall.Tsession) {
	sc.lock.Lock()
