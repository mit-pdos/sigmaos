...
func (st *SessionTable) Alloc(sid np.Tsession, conn protsrv.NetConn, replies chan *np.Fcall) *Session {
	st.Lock()
	defer st.Unlock()

	if sess, ok := st.sessions[sid]; ok {
		sess.maybeSetConn(conn, replies)
		return sess
	}
	sess := makeSession(conn, st.mkps(st.fssrv, sid), sid, replies, st.tm.AddThread())
