...
func (c *VimCommand) Usage() string {
	return "vim <filename>"
}
func (c *VimCommand) Execute(ctx *shellctx.ShellContext, args []string, stdin io.Reader, stdout io.Writer, stderr io.Writer) bool {
	if len(args) != 1 {
	...
	if err := cmd.Run(); err != nil {
		fmt.Fprintf(stderr, "error running vim: %v", err)
		return false
	}
