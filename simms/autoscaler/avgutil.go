...
func NewAvgUtilAutoscaler(t *uint64, asp *AvgUtilAutoscalerParams, svc *simms.Microservice) *AvgUtilAutoscaler {
	return &AvgUtilAutoscaler{
		t:              t,
		svc:            svc,
		targetUtil:     asp.TargetUtil,
		scaleFrequency: asp.ScaleFreq,
		run:            false,
		nScaleUp:       0,
		nScaleDown:     0,
	}
...
func (ua *AvgUtilAutoscaler) getScalingDecision(istats []*simms.ServiceInstanceStats) (scalingDecision, int) {
	...
	currentUtil := avgUtil(readyIStats)
	desiredNReplicas := k8sCalcDesiredNReplicas(currentNReplicas, currentUtil, ua.targetUtil, DEFAULT_TOLERANCE)
