...
func TestPutfile(t *testing.T) {
	b := []byte("hello")
	fence := &fcall.Tfence{Fenceid: &fcall.Tfenceid{Path: 36, Serverid: 2}, Epoch: 7}
	msg := sp.MkTputfile(1, sp.OWRITE, 0777, 101, []string{"f"}, b)
	fcall := sp.MakeFcallMsg(msg, 0, 13, nil, &fcall.Tinterval{Start: 1, End: 2}, fence)
	frame, error := MarshalFcallMsgByte(fcall)
	assert.Nil(t, error)
	fc, error := UnmarshalFcallMsg(frame)
	assert.Nil(t, error)
	fcall1 := fc.(*fcall.FcallMsg)
