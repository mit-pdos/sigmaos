...
if docker ps -a | grep -qE "$USER_IMAGE_NAME|$KERNEL_IMAGE_NAME|$DB_IMAGE_NAME|$MONGO_IMAGE_NAME|cgroups-hotel-imgresize"; then
  for container in $(docker ps -a | grep -E "$USER_IMAGE_NAME|$KERNEL_IMAGE_NAME|$DB_IMAGE_NAME|$MONGO_IMAGE_NAME|cgroups-hotel-imgresize" | cut -d ' ' -f1) ; do
