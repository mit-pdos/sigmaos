...
if [[ $APPS == "--apps" ]]; then
    if [[ $FAST == "--fast" ]]; then
        PKGS="apps/mr apps/imgresize kv apps/hotel apps/socialnetwork"
...
#        cleanup
        ...
        for T in apps/imgresize apps/mr apps/hotel apps/socialnetwork www; do
            if ! [ -z "$SKIPTO" ]; then
              if [[ "$SKIPTO" == "$T" ]]; then
                # Stop skipping
                SKIPTO=""
