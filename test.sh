...
if [[ $COMPILE == "--compile" ]]; then
    ...
    for T in path intervals serr linuxsched perf sigmap sessclnt proxy reader writer stats fslib semclnt electclnt fslib memfs named procclnt ux s3 bootkernelclnt leaderclnt leadertest kvgrp cachedsvcclnt www fslibsrv realmclnt auth mr imgresizesrv kv hotel socialnetwork benchmarks; do
