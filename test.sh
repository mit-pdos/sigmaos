...
if [[ $BASIC == "--basic" ]]; then
    ...
    for T in namesrv util/coordination/semclnt chunk/srv sigmaclnt/procclnt proxy/ux bootkernelclnt proxy/s3 ft/leaderclnt leadertest apps/kv/kvgrp apps/cache/cachegrp/clnt; do
        if ! [ -z "$SKIPTO" ]; then
          if [[ "$SKIPTO" == "$T" ]]; then
            # Stop skipping
            SKIPTO=""
