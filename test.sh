...
if [[ $BASIC == "--basic" ]]; then
    ...
    for T in namesrv semclnt chunk/srv procclnt ux bootkernelclnt proxy/s3 leaderclnt leadertest apps/kv/kvgrp apps/cache/cachegrp/clnt; do
        if ! [ -z "$SKIPTO" ]; then
          if [[ "$SKIPTO" == "$T" ]]; then
            # Stop skipping
            SKIPTO=""
