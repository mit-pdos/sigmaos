...
func tspSingleRecursive(g *Graph, homeNode int, choices *Set, currentNode int) (int, []int, error) {
	...
	if len(*choices) == 1 {
		// XXX This is taking up the most resources.
...
// This version is faster (~1.5x) for Multi but slower (~1.5x) for Single.
// The speedup was not worth the decrease in readability.
//
//func tspSingleRecursive(g *Graph, homeNode int, choices *Set, currentNode int) (int, []int, error) {
//	//fmt.Printf("%v: %v\n", currentNode, choices)
//	if len(*choices) == 0 {
